...
export class Service {
  ...
  async create (data, params) {
    ...
    if (!data.subscriptionProperties) throw new BadRequest('feathers-webpush:create: expected missing \'data.subscriptionProperties\' parameter')
    debug(`method 'create' called with 'dataNotification': ${data.dataNotification}, 'subscriptionService': ${data.subscriptionService}, 'subscriptionFilter': ${data.subscriptionFilter} and 'subscriptionProperties': ${data.subscriptionProperties}`)

    // Set data
    let { dataNotification, subscriptionService, subscriptionProperties, subscriptionFilter = {} } = data
    ...
    _.forEach(subscriptionsService, subscriptionService => {
      _.isArray(subscriptionService[subscriptionProperties]) ? subscriptions.push(...subscriptionService[subscriptionProperties]) : subscriptions.push(subscriptionService[subscriptionProperties])
    })
