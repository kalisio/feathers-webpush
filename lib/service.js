...
export class Service {
  ...
  async create (data, params) {
    ...
    if (!data.subscriptionCollection) throw new BadRequest('feathers-webpush:create: expected missing \'data.subscriptionCollection\' parameter')
    debug(`method 'create' called with 'dataNotification': ${data.dataNotification}, 'subscriptionService': ${data.subscriptionService}, 'subscriptionFilter': ${data.subscriptionFilter} and 'subscriptionCollection': ${data.subscriptionCollection}`)

    // Set data
    let { dataNotification, subscriptionService, subscriptionCollection, subscriptionFilter = {} } = data
    ...
    _.forEach(subscriptionsService, subscriptionService => {
      _.isArray(subscriptionService[subscriptionCollection]) ? subscriptions.push(...subscriptionService[subscriptionCollection]) : subscriptions.push(subscriptionService[subscriptionCollection])
    })
